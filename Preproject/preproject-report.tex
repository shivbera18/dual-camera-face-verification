...
% Chapter 5: Implementation Plan and Datasets
\chapter{Implementation Plan and Datasets}

\section{Hardware Requirements}

\subsection{Camera Setup}

\textbf{Cameras}: 2× Logitech C270 USB webcams (720p, 30fps) or equivalent
\begin{itemize}
\item Resolution: 1280×720 pixels
\item Frame rate: 30 FPS
\item Interface: USB 2.0 or 3.0
\item Cost: ₹1,200-1,500 each (total: ₹2,400-3,000)
\end{itemize}

\textbf{Mounting}: Rigid bracket maintaining 6-10cm baseline
\begin{itemize}
\item Material: Wood, metal, or 3D-printed plastic
\item Requirements: Parallel alignment, no movement during operation
\item Cost: ₹200-500
\end{itemize}

\textbf{Calibration}: Printed checkerboard pattern (9×6 inner corners, 25mm squares)

\subsection{Computing Hardware}

\textbf{Minimum requirements}:
\begin{itemize}
\item CPU: Intel Core i5 / AMD Ryzen 5 or better
\item RAM: 8 GB
\item Storage: 20 GB for models and datasets
\item USB ports: 2× USB 2.0 or better
\item OS: Windows 10/11 or Ubuntu 20.04+
\end{itemize}

\textbf{Recommended for training}:
\begin{itemize}
\item GPU: NVIDIA GTX 1050 or better (4GB VRAM)
\item RAM: 16 GB
\item Storage: 50 GB
\end{itemize}
...
\begin{itemize}
\item CPU-only inference: 10-15 FPS
\item GPU inference: 25-30 FPS
\item Training time (GPU): 2-4 hours for deepfake detector, 10 minutes for anti-spoofing
\end{itemize}
...
\textbf{Purpose}: Training texture-based anti-spoofing (LBP + SVM)
...
\item Videos: 1,300
\item Subjects: 50
\item Attack types: Printed photos, video replay on screens
\item Conditions: Controlled and adverse lighting
\end{itemize}

\textbf{Data split}:
\begin{itemize}
\item Training: 360 videos (180 real + 180 attack)
\item Validation: 360 videos
\item Test: 480 videos
\end{itemize}

\textbf{Usage}: Extract frames, compute LBP features, train SVM classifier

\subsection{FaceForensics++ Dataset}

\textbf{Purpose}: Training deepfake detector (EfficientNet-B0)

\textbf{Statistics}:
\begin{itemize}
\item Size: 3 GB (face crops only, not full videos)
\item Videos: 5,000 (1,000 real + 4,000 manipulated)
\item Manipulation types: DeepFakes, Face2Face, FaceSwap, NeuralTextures
\item Quality levels: High quality (c23 compression)
\end{itemize}

\textbf{Data split}:
\begin{itemize}
\item Training: 720 videos per manipulation type
\item Validation: 140 videos per manipulation type
\item Test: 140 videos per manipulation type
\end{itemize}

\textbf{Download}: Face crops only (not full videos) to save storage

\subsection{LFW Dataset}

\textbf{Purpose}: Evaluating face verification accuracy
...
\item Images: 13,233
\item Subjects: 5,749
\item Pairs: 6,000 (3,000 matched, 3,000 mismatched)
\end{itemize}

\textbf{Usage}: Benchmark ArcFace performance (target: >99\% accuracy)

\subsection{Custom Dataset}

\textbf{Purpose}: Testing specific camera setup and demo

\textbf{Plan}: Record 10-20 subjects (friends, family with permission)
\begin{itemize}
\item Real faces: 3-5 videos per person (different angles, distances)
\item Photo attacks: Phone display, printed photos
\item Video attacks: Laptop replay, tablet replay
\item Total size: 1-2 GB
\end{itemize}
...
\begin{table}[h]
\centering
\caption{12-Week Implementation Schedule}
\begin{tabular}{|l|l|}
\hline
\textbf{Week} & \textbf{Tasks} \\
\hline
1-2 & Hardware setup, stereo calibration \\
3 & Stereo depth computation (SGBM) \\
4 & Face detection integration (RetinaFace) \\
5 & Depth-based liveness detection \\
6 & Texture anti-spoofing training (LBP+SVM) \\
7-8 & Deepfake detector training (EfficientNet) \\
9 & Face recognition integration (ArcFace) \\
10 & System integration, LoRA optimization \\
11 & Testing, custom dataset recording \\
...
% Chapter 6: Expected Results and Evaluation
\chapter{Expected Results and Evaluation Metrics}

\section{Performance Targets}

\subsection{Anti-Spoofing Performance}

\begin{table}[h]
\centering
\caption{Anti-Spoofing Performance Targets}
\begin{tabular}{|l|c|c|}
\hline
\textbf{Metric} & \textbf{Target} & \textbf{Excellent} \\
\hline
Accuracy & >95\% & >98\% \\
...
\hline
\end{tabular}
\label{tab:antispoofing-targets}
\end{table}

\textbf{APCER} (Attack Presentation Classification Error Rate): Percentage of attack samples incorrectly classified as real

\textbf{BPCER} (Bona Fide Presentation Classification Error Rate): Percentage of real samples incorrectly classified as attacks

\textbf{ACER} (Average Classification Error Rate): $(APCER + BPCER) / 2$

\subsection{Deepfake Detection Performance}

\begin{table}[h]
\centering
\caption{Deepfake Detection Performance Targets}
\begin{tabular}{|l|c|c|}
\hline
\textbf{Metric} & \textbf{Target} & \textbf{Excellent} \\
\hline
...
\hline
\end{tabular}
\label{tab:deepfake-targets}
\end{table}

\subsection{Face Verification Performance}

\begin{table}[h]
\centering
\caption{Face Verification Performance Targets}
\begin{tabular}{|l|c|}
\hline
...
\hline
\end{tabular}
\label{tab:verification-targets}
\end{table}
...
\begin{table}[h]
\centering
\caption{System-Level Performance Targets}
\begin{tabular}{|l|c|}
\hline
\textbf{Metric} & \textbf{Target} \\
\hline
...
Memory usage & <2 GB \\
CPU usage & <80\% \\
Model size (with LoRA) & <5 MB \\
\hline
\end{tabular}
\label{tab:system-targets}
\end{table}
...
\hline
\textbf{Method} & \textbf{Accuracy} & \textbf{FAR} & \textbf{Speed} & \textbf{Cost} \\
\hline
Single Camera + CNN & 92\% & 3.5\% & 30 FPS & \$50 \\
Depth Sensor & 98\% & 0.5\% & 15 FPS & \$300 \\
\textbf{Our Method} & \textbf{>95\%} & \textbf{<2\%} & \textbf{>20 FPS} & \textbf{\$60} \\
\hline
\end{tabular}
\label{tab:comparison}
\end{table}

Our dual-camera approach achieves accuracy approaching depth sensor systems at 1/5 the cost, while maintaining real-time performance superior to depth sensors.

\section{Evaluation Methodology}

\subsection{Anti-Spoofing Evaluation}

\textbf{Test protocol}:
\begin{enumerate}
\item Evaluate on Replay-Attack test set (480 videos)
\item Compute APCER, BPCER, ACER metrics
\item Generate ROC curves for depth and texture modalities
\item Analyze failure cases (which attacks succeed)
\end{enumerate}

\subsection{Deepfake Detection Evaluation}

\textbf{Test protocol}:
\begin{enumerate}
\item Evaluate on FaceForensics++ test set (560 videos)
\item Compute accuracy, precision, recall, AUC
\item Test cross-dataset generalization on Celeb-DF
\item Analyze per-manipulation-type performance
\end{enumerate}

\subsection{Face Verification Evaluation}

\textbf{Test protocol}:
\begin{enumerate}
\item Evaluate on LFW benchmark (6,000 pairs)
\item Compute verification accuracy at threshold $\tau=0.6$
\item Generate ROC curve, compute AUC
\item Measure FAR at various FRR operating points
\end{enumerate}
...
\begin{enumerate}
\item \textbf{Depth only}: Disable texture anti-spoofing
\item \textbf{Texture only}: Disable depth analysis
\item \textbf{No deepfake detection}: Measure impact on AI-generated faces
\item \textbf{Without LoRA}: Compare full fine-tuning vs LoRA
\end{enumerate}

Expected findings: Multi-modal fusion (depth + texture) provides 3-5\% accuracy improvement over single modality. LoRA causes <1\% accuracy drop while reducing model size by 35×.
...
\begin{itemize}
\item \textbf{Hardware-software co-design}: Stereo vision using \$60 webcams achieves depth perception equivalent to \$300 sensors

\item \textbf{Multi-modal security}: Combining depth analysis, texture analysis (LBP), and deep learning (EfficientNet-B0) provides defense-in-depth against presentation attacks and deepfakes

\item \textbf{State-of-the-art components}: RetinaFace for accurate face detection, EfficientNet-B0 for efficient deepfake detection, ArcFace for high-accuracy verification

\item \textbf{Parameter-efficient adaptation}: LoRA enables 35× model compression with <1\% accuracy loss, facilitating edge deployment

\item \textbf{Real-time performance}: 15-30 FPS on standard hardware without GPU acceleration
\end{itemize}

\textbf{Expected contributions}:

\begin{itemize}
\item Empirical demonstration that dual-camera stereo vision provides cost-effective alternative to depth sensors for face anti-spoofing

\item Open-source implementation enabling reproducibility and further research

\item Analysis of LoRA's effectiveness for compressing face verification models

\item Practical system suitable for real-world deployment in access control, mobile authentication, and IoT security
\end{itemize}
...
\textbf{3D mask detection}: Current depth analysis detects flat attacks but may be vulnerable to high-quality 3D masks. Future work will analyze depth distribution patterns and texture consistency to detect masks.

\textbf{Continuous authentication}: Extend from single-shot verification to continuous monitoring throughout user sessions, detecting session hijacking attempts.

\textbf{Multi-user scenarios}: Handle multiple faces in frame, performing parallel verification for group authentication scenarios.

\subsection{Medium-Term (6-12 months)}

\textbf{Federated learning}: Enable privacy-preserving model updates by training on decentralized user devices without sharing raw biometric data.

\textbf{Adversarial robustness}: Evaluate and improve robustness against adversarial perturbations designed to fool the system.

\textbf{Cross-dataset generalization}: Improve deepfake detector generalization to unseen manipulation methods through domain adaptation techniques.

\subsection{Long-Term (1-2 years)}

\textbf{Vision Transformers}: Explore replacing CNN backbones with Vision Transformers for improved accuracy and interpretability through attention mechanisms.

\textbf{Neural Architecture Search}: Automatically discover optimal architectures for the specific dual-camera setup and deployment constraints.

\textbf{Multimodal fusion}: Integrate additional modalities (voice, gait, keystroke dynamics) for comprehensive continuous authentication.

\section{Societal Impact and Ethical Considerations}

\textbf{Positive impacts}:
\begin{itemize}
\item Enhanced security for financial transactions, reducing fraud
\item Improved access control for sensitive facilities
\item Convenient authentication eliminating password burdens
\item Cost-effective solution enabling widespread deployment
\end{itemize}

\textbf{Ethical considerations}:
\begin{itemize}
\item \textbf{Privacy}: Biometric data is sensitive and permanent. System design must ensure data protection through encryption and local processing where possible.

\item \textbf{Bias and fairness}: Face recognition systems have exhibited demographic disparities. Evaluation must include diverse populations, and mitigation strategies must address any identified biases.

\item \textbf{Consent and transparency}: Users must provide informed consent for biometric data collection. System operation should be transparent, with clear explanations of decisions.

\item \textbf{Security vs. convenience}: Threshold selection must balance security requirements with user experience, avoiding excessive false rejections that frustrate legitimate users.
\end{itemize}

\section{Conclusion}

The proposed dual-camera face verification system represents a significant advance in biometric security, addressing fundamental limitations of single-camera systems through innovative hardware-software co-design. By combining stereo vision, multi-modal anti-spoofing, state-of-the-art deep learning, and parameter-efficient adaptation, the system achieves high accuracy, real-time performance, and cost-effectiveness suitable for widespread deployment.

The 12-week implementation plan provides a clear roadmap from hardware setup through model training to system integration and evaluation. Expected results demonstrate >95\% attack detection accuracy with <2\% false rejection rate, approaching the performance of expensive depth sensor systems at 1/5 the cost.

This work contributes to the broader goal of secure, convenient, and accessible biometric authentication, with potential impact across consumer electronics, financial services, access control, and IoT security. Success requires not only technical excellence but also responsible development addressing fairness, privacy, and ethical deployment to serve human needs while respecting individual rights.
