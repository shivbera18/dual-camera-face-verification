...
\caption{LoRA architecture showing low-rank decomposition matrices injected into pre-trained model layers}
...
\section{Datasets and Training Strategy}

\subsection{Replay-Attack Dataset}

Used for training texture-based anti-spoofing (LBP + SVM). Contains 1,300 videos (4 GB) with 50 subjects under controlled and adverse lighting. Attack types include printed photos and video replay on screens. Training uses 360 videos (180 real + 180 attack), with 480 videos reserved for testing.

\subsection{FaceForensics++ Dataset}

Used for training the EfficientNet-B0 deepfake detector. Contains 5,000 videos (3 GB face crops) with four manipulation types: DeepFakes, Face2Face, FaceSwap, and NeuralTextures. Training employs transfer learning from ImageNet pre-trained weights, fine-tuning on 720 videos per manipulation type for 15 epochs with LoRA adaptation ($r=8$). This achieves 94-95\% accuracy with only 150K trainable parameters.

\subsection{LFW Dataset}

Used for benchmarking face verification accuracy. Contains 13,233 images across 5,749 subjects with 6,000 evaluation pairs (3,000 matched, 3,000 mismatched). Pre-trained ArcFace models achieve >99\% accuracy on this benchmark without additional training.

\section{Performance Targets and Evaluation}

The system targets >95\% anti-spoofing accuracy with <2\% Attack Presentation Classification Error Rate (APCER) and <3\% Bona Fide Presentation Classification Error Rate (BPCER). For deepfake detection, the target is >93\% accuracy with >0.95 AUC. Face verification aims for >99\% accuracy on LFW benchmark. System-level performance targets include <200ms end-to-end latency, >15 FPS throughput, and <5 MB model size with LoRA adaptation.
...
The dual-camera approach achieves accuracy approaching depth sensor systems at 1/5 the cost while maintaining superior real-time performance. Evaluation will be conducted on Replay-Attack (480 test videos), FaceForensics++ (560 test videos), and LFW (6,000 pairs) benchmarks. Ablation studies will quantify each component's contribution, with expected findings showing multi-modal fusion (depth + texture) provides 3-5\% accuracy improvement over single modality, while LoRA causes <1\% accuracy drop with 35× model size reduction.
