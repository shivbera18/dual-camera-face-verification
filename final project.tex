...
\usepackage[bookmarks, colorlinks=false, pdfborder={0 0 0}, pdftitle={Dual Camera Face Verification with Deepfake Detection}, pdfauthor={Shivratan Bera}, pdfsubject={Final Project Report}]{hyperref}
...
\vspace{0.5cm}
{\large SHIVRATAN BERA (Enrollment No: 2022BECE103)\par}
{\large ARSALAN BASHIR (Enrollment No: 2022BECE057)\par}
...
We would like to express our sincere gratitude to all those who have contributed to the successful completion of this final project report. We extend our deepest appreciation to our project guide \textbf{Professor A. A. Mir} for his invaluable guidance, continuous support, and insightful feedback throughout the research, implementation, and evaluation phases. His mentorship has been the cornerstone of our academic endeavor.

We are extremely grateful to the \textbf{Department of Electronics and Communication, National Institute of Technology, Srinagar} for providing us with the necessary resources, equipment, and laboratory facilities to pursue this research without any hindrance. We also extend our thanks to the faculty members and support staff whose assistance made this project possible.
...
\begin{tikzpicture}[node distance=1.5cm, auto,
    box/.style={rectangle, draw=black, thick, fill=gray!10, text width=5cm, align=center, minimum height=1.5cm, rounded corners},
    arrow/.style={thick, -{Latex[length=3mm, width=2mm]}}]
    
    \node[box] (input) {Dual Webcam Input\\(Synchronized Parallel Frames)};
    \node[box, below=1cm of input] (retina) {Face Processing Layer\\(RetinaFace Localization)};
    \node[box, below=1.5cm of retina, xshift=-3cm] (effnet) {Deepfake Evaluation\\(LoRA EfficientNet B0)};
    \node[box, right=1cm of effnet] (arcface) {Identity Extraction\\(ArcFace 512D Embedding)};
    \node[box, below=1.5cm of effnet, xshift=3cm] (fusion) {Score Fusion Engine\\(Mean Fake, Max Match)};
    \node[box, below=1cm of fusion] (decision) {Final Execution Decision\\(Accept or Reject)};
    
    \draw[arrow] (input) -- (retina);
    \draw[arrow] (retina) -- ++(0,-1cm) -| (effnet);
    \draw[arrow] (retina) -- ++(0,-1cm) -| (arcface);
    \draw[arrow] (effnet) |- (fusion);
    \draw[arrow] (arcface) |- (fusion);
    \draw[arrow] (fusion) -- (decision);
\end{tikzpicture}
\caption{System Architecture Flowchart mathematically detailing parallel neural module execution and probabilistic metric fusion logic.}
\label{fig:arch_flowchart}
\end{figure}

The five core structural software layers are defined exactly as:
\begin{enumerate}
    ...
    \item \textbf{Face Processing Layer:} Executes advanced facial detection, geometric spatial alignment, and standardized mathematical pixel cropping utilizing the RetinaFace backbone.
    \item \textbf{Model Layer:} Probabilistically evaluates the genuine visual authenticity via LoRA EfficientNet B0 and simultaneously calculates the deep identity embedding matrix via ArcFace.
    \item \textbf{Decision Layer:} Mathematically fuses the parallel predictive metrics and strictly enforces rigid acceptance probability thresholds.
    \item \textbf{Storage and Logging Layer:} Persists identity enrollment geometric templates securely and permanently logs all forensic authentication attempts for systemic review.
\end{enumerate}

\section{Dual Camera Runtime Flow and Threading}
The dual camera inference protocol relies heavily on evaluating massive parallel video streams completely seamlessly. This advanced runtime flow explicitly executes the baseline neural network layer completely independently across both parallel optical views before mathematically fusing the probabilistic scores.
...
\subsection{Parallel Evaluation and Statistical Score Fusion}
Once a perfectly synchronized dual frame pair is successfully captured, the software executes the facial processing layer fully independently on both the left frame buffer and the right frame buffer. 

EfficientNet mathematically outputs a synthetic probability score independently for both frames. The mathematical statistical fusion logic directly calculates the absolute mean average of both frame scores to comprehensively determine the final algorithmic fake probability. Simultaneously, ArcFace outputs a massive mathematical identity vector independently for both frames. The fusion logic calculates the strict mathematical maximum cosine similarity score against the encrypted database vector.
