...
\usepackage[bookmarks, colorlinks=false, pdfborder={0 0 0}, pdftitle={Dual Camera Face Verification System with Stereo Vision Based Liveness Detection and Deepfake Prevention}, pdfauthor={Shivratan, Arsalan Bashir}, pdfsubject={Final Project Report}]{hyperref}
\usepackage{times} % Explicitly enforce Times New Roman font
\usepackage{titlesec}
% Remove the massive default vertical gap above chapter titles
\titleformat{\chapter}[display]
  {\normalfont\huge\bfseries\filcenter}{\chaptertitlename\ \thechapter}{20pt}{\Huge}
\titlespacing*{\chapter}{0pt}{0pt}{40pt}
\usepackage{fancyhdr}

% Configure consistent headers with page numbers
\setlength{\headheight}{15pt}
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{Dual Camera Face Verification System}
\fancyhead[R]{\thepage}
\renewcommand{\headrulewidth}{0.4pt}

% Force chapter-starting pages to also use the header format
...
{\large \textit{By}\par}
\vspace{0.2cm}
{\large \textbf{Shivratan} (2022BECE103) \hspace{1cm} \textbf{Arsalan Bashir} (2022BECE057)\par}

\vspace{1cm}
{\large \textit{Under the Guidance of}\par}
\vspace{0.2cm}
{\large \textbf{Prof. Aijaz Ahmed Mir}\par}
\includegraphics[width=0.25\linewidth]{images.jpg}
\vfill
...
It is to certify that the contents of the report entitled ``Dual Camera Face Verification System with Stereo Vision Based Liveness Detection and Deepfake Prevention'' is a bonafide work carried out by Mr. Shivratan (2022BECE103) and Mr. Arsalan Bashir (2022BECE057) under my supervision and guidance in partial fulfillment of the requirements for the award of the degree of Bachelor of Technology in Electronics \& Communication Engineering. The contents of the report have not been submitted earlier for the award of any other degree or certificates and I hereby commend the work done by them in this connection.
...
This project titled ``Dual Camera Face Verification System with Stereo Vision Based Liveness Detection and Deepfake Prevention'' carried out by Mr. Shivratan (2022BECE103) and Mr. Arsalan Bashir (2022BECE057), is hereby approved as a creditable study of technology in Electronics \& Communication Engineering and is presented in a satisfactory manner.
...
We are also thankful to the Head of Department, Electronics \& Communication Engineering, and all faculty members for providing us with the necessary facilities and computational tools required for the successful completion of this work. We acknowledge the computational resources made available by the department.
...
\vspace{1.5cm}
\begin{flushright}
\textbf{Shivratan} (2022BECE103)\\
\textbf{Arsalan Bashir} (2022BECE057)\\
\end{flushright}
...
\section{Dataset Composition and Forensics Architecture}
The primary training corpus for evaluating the algorithmic deepfake detection module heavily utilizes the renowned \textbf{FaceForensics++} dataset. We actively supplemented this dataset alongside various custom local captures to ensure severe lighting and resolution diversity. 

\subsection{The FaceForensics++ Algorithms}
The FaceForensics++ dataset was specifically chosen because it provides a meticulously controlled environment containing manipulated videos generated by four distinctly different generative methods. Training on this diversity is what prevents the LoRA model from memorizing a single artifact, ensuring robust generalization against zero-day attacks in the real world:
\begin{itemize}
    \item \textbf{Deepfakes:} Utilizes classic Autoencoders to replace faces, prone to severe temporal flickering and skin-tone mismatch at the jawline boundaries.
    \item \textbf{Face2Face:} A classical computer graphics approach that fits a 3D Morphable Face Model to the target to transfer expressions, altering mouth geometry but leaving the target's identity intact.
    \item \textbf{FaceSwap:} A 3D graphics method transferring the inner facial region and blending it using Poisson Image Editing.
    \item \textbf{NeuralTextures:} Employs Generative Adversarial Networks (GANs) to render photorealistic mouth movements directly into the target video matrix.
\end{itemize}
...
\section{Aggressive Augmentation Protocol (Albumentations)}
To actively prevent catastrophic overfitting on the training set, image augmentations heavily employ the advanced \textbf{Albumentations} python library. Neural networks have a dangerous tendency to memorize background textures rather than generalized facial structures. The protocol systematically applies Horizontal Flips (pose invariance), Extreme JPEG Compression (mimicking social media artifacting), Gaussian Noise (replicating cheap camera grain), and Coarse Dropout (randomly dropping black squares to force the network to learn holistic structures).
...
Choosing the correct dataset is absolutely paramount for training robust biometric models capable of generalizing to unseen real world scenarios. Every core module in this pipeline was trained on specifically chosen academic datasets.

\subsection{RetinaFace (Localization) - WIDER FACE}
For the facial extraction module, RetinaFace was pretrained heavily on the \textbf{WIDER FACE} dataset. 

\textbf{Why it was used:} This dataset contains over 32,000 images with extreme crowds and variations in scale, pose, and occlusion. It features faces heavily obscured by sunglasses, surgical masks, and extreme profile angles. Training on WIDER FACE ensures our system's localization layer never fails, even in harsh real-world desktop environments.

\subsection{ArcFace (Identity) - MS-Celeb-1M}
For the identity verification module, ArcFace utilized the massive \textbf{MS-Celeb-1M} dataset for training and the \textbf{Labeled Faces in the Wild} dataset for strict validation.

\textbf{Why it was used:} MS-Celeb-1M contains millions of images spanning 100,000 distinct identities. This immense scale is mathematically required to train ArcFace's Additive Angular Margin loss. Without millions of identities, the neural network cannot properly shape the high-dimensional hypersphere geometry needed to map open-set faces.

\subsection{EfficientNet (Deepfakes) - FaceForensics++}
For our primary deepfake contribution, we exclusively selected \textbf{FaceForensics++}.

\textbf{Why it was used:} By forcing the LoRA bottleneck to train on Deepfakes, Face2Face, FaceSwap, and NeuralTextures simultaneously, the network learns the fundamental mathematical signature of synthetic generation rather than memorizing a single algorithm.
...
\bibitem{hirschmuller2008sgbm}
Hirschmuller, H. (2008). Stereo Processing by Semiglobal Matching and Mutual Information. IEEE Transactions on Pattern Analysis and Machine Intelligence.

\bibitem{yang2016widerface}
Yang, S., Luo, P., Loy, C. C., and Tang, X. (2016). WIDER FACE: A Face Detection Benchmark. IEEE Conference on Computer Vision and Pattern Recognition (CVPR).
...
Loshchilov, I., and Hutter, F. (2017). Decoupled Weight Decay Regularization (AdamW). International Conference on Learning Representations (ICLR).

\bibitem{perov2020deepfacelab}
Perov, I., Gao, D., Chervoniy, V., Liu, K., Marangonda, S., Umé, C., Diks, M., Keller, C. G., Jiang, S., and Zhang, S. (2020). DeepFaceLab: Integrated, flexible and extensible face-swapping framework. arXiv preprint.
\end{thebibliography}
